...
def list():
    resp = requests.get("http://"+get_first_broker(KAFKA_BROKERS)+":8083/connectors")
    if resp.status_code != 200:
        raise ApiError('GET /connectors {}'.format(resp.status_code))
    if len(resp.json()) != 0:
...
def create(json_file_name):
    if os.path.exists(json_file_name):
        ...
        resp = requests.post("http://"+get_first_broker(KAFKA_BROKERS)+":8083/connectors", json=connector_properties)
        if resp.status_code != 200:
            print "Error "+str(resp.status_code)+" "+str(resp.json())
            print "(with the request : http://"+get_first_broker(KAFKA_BROKERS)+":8083/connectors/)"
...
def get(connector_name):
    resp = requests.get("http://"+get_first_broker(KAFKA_BROKERS)+":8083/connectors/"+connector_name)
    if resp.status_code != 200:
        print "Error "+str(resp.status_code)+" "+resp.json()['message']
        print "(with the request : http://"+get_first_broker(KAFKA_BROKERS)+":8083/connectors/"+connector_name+")"
    elif resp.status_code == 200 and len(resp.json()) != 0:
        name = resp.json()['name']
...
def status(connector_name):
    resp = requests.get("http://"+get_first_broker(KAFKA_BROKERS)+":8083/connectors/"+connector_name+"/status")
    if resp.status_code != 200:
        print "Error "+str(resp.status_code)+" "+resp.json()['message']
        print "(with the request : http://"+get_first_broker(KAFKA_BROKERS)+":8083/connectors/"+connector_name+"/status)"
    elif resp.status_code == 200 and len(resp.json()) != 0:
        for todo_item in resp.json():
            print(todo_item)
...
def get_config(connector_name):
    print "config of "+connector_name

def update_config(connector_name, conf_file_name=None):
    print "alter config of "+connector_name
...
def tasks_list(connector_name):
    resp = requests.get("http://"+get_first_broker(KAFKA_BROKERS)+":8083/connectors/"+connector_name+"/tasks")
    if resp.status_code != 200:
        print "Error "+str(resp.status_code)+" "+resp.json()['message']
        print "(with the request : http://"+get_first_broker(KAFKA_BROKERS)+":8083/connectors/"+connector_name+"/tasks)"
    elif resp.status_code == 200 and len(resp.json()) != 0:
        for todo_item in resp.json():
...
def tasks_status(connector_name, task_id):
    resp = requests.get("http://"+get_first_broker(KAFKA_BROKERS)+":8083/connectors/"+connector_name+"/tasks/"+task_id+"/status")
    if resp.status_code != 200:
        print "Error "+str(resp.status_code)+" "+resp.json()['message']
        print "(with the request : http://"+get_first_broker(KAFKA_BROKERS)+":8083/connectors/"+connector_name+"/tasks/"+task_id+"/status)"
    elif resp.status_code == 200 and len(resp.json()) != 0:
        print "Task "+task_id
        for todo_item in resp.json():
...
def tasks_restart(connector_name, task_id):
    resp = requests.post("http://"+get_first_broker(KAFKA_BROKERS)+":8083/connectors/"+connector_name+"/tasks/"+task_id+"/restart")
